...
def search():
    if request.method == "POST":
        ...
        # Search in the users table for biography
        users_cursor = db.execute(
            text(
                """
                SELECT username, biography FROM users
                WHERE biography ILIKE :search_query
                """
            ), { "search_query": f"%{search_query}%" }
        )
        users_with_biography = [list(el) for el in users_cursor]
        ...
        context = {
            ...
            "users_with_biography": users_with_biography
        }
